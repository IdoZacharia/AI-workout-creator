const [isLoading, setIsLoading] = useState(false);
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      setWorkoutPlan(result); // Store the AI's response
  ...
  return (
    ...
    <div className="min-h-screen bg-slate-900 flex flex-col items-center justify-center p-4 text-white">
      ...
      {/* Only render if workoutPlan is not null */}
      {workoutPlan && (
        <div className="mt-16 space-y-12 pb-24 px-4 max-w-3xl mx-auto">
          {/* Header */}
          ...
          {/* Vertical Stack - One Day Per Row */}
          <div className="flex flex-col gap-12">
            {workoutPlan.weekly_routine.map((day, index) => (
              ...
              >
                ...
                {/* Exercises - Now in a spacious list */}
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4 bg-slate-950/50 p-4 rounded-2xl border border-slate-800/50">
                        <div>
                          ...
                          <p className="text-xs text-emerald-400 font-semibold">
                            {ex.recommended_weight || "Adjust based on RPE"}
                          </p>
                        </div>
                        <div>
                          <p className="text-[10px] text-slate-500 uppercase font-bold mb-1">Pro Tip</p>
                          <p className="text-xs text-slate-400 italic">
                            "{ex.coaching_tip || "Maintain strict form throughout."}"
                          </p>
                        </div>
                      </div>
                    </div>
